...
def is_NE(token, sep, tag_types):
  if sep in token:
    return token.split(sep)[tag_types.index('ner')+1 if 'ner' in tag_types else 1] != 'O'
  return False
...
def main(args):
  tokenizer = SentencePiece(args.tokenizer) if args.tokenizer else None
  ...
  for sentence in sentences:
    tokenized = tokenize(tokenizer, sentence, args.sep, args.tag_types)
    if args.print:
      yield " ".join(args.sep.join(t for t in token) for token in tokenized)
    if args.ner_stats:
      token_count += len(tokens)
      sentence_count += 1
      NE_count += sum(is_NE(token, args.sep, args.tag_types) for token in tokens)
...
parser.add_argument("--tag-types", nargs="*",
                    help="comma separated list of tag types for each tag channel in the input - this determines how tags are broadcast across subwords")
